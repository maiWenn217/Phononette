...
all_contexts = pd.read_csv("./context.csv")
if shuffle :
...
for chunk_index in range(0, len(all_contexts) // chunk_size ) : 
  ...
  while json_entry_index < len(json_entries) :
    ...
    prompt = f"""L'enregistrement en JSON suivant : {json_entry} contient un mot du français, une catégorie, une définition, une étymologie et une famille de mots. Construis un nouvel enregistrement JSON qui contient les réponses aux questions suivantes : 
    ...
    * Q2 = quel est l'affixe du mot d'après l'étymologie ? (répondre une liste d'un seul élément s'il n'y a qu'un seul affixe ou une liste de plusieurs éléments s'il y a plusieurs affixes ou une liste vide si le mot n'est pas dérivé)
    * Q3 = quel est le radical du mot ? (répondre null si le mot n'est pas dérivé)
    * Q4 = quels sont les étymons qu'on retrouve dans l'étymologie et quel est leur sens ? (répondre une liste de couples étymon, sens ou une liste vide si le mot n'est pas dérivé)
  ...
  if len(full_df) == 0 :
    ...
    full_df.to_csv(f"./Mistral_responses_{'shuffled' if shuffle else 'unshuffled'}.csv", index=False)
  else :
