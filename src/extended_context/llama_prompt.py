...
MODEL_NAME = "meta-llama/Llama-3.1-8B-Instruct" # à changer selon le modèle
auth_token = "XXX" # remplacer XXX par le token HF

device = "cuda" if torch.cuda.is_available() else "cpu"

tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, trust_remote_code=True, token=auth_token)
model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, trust_remote_code=True, token=auth_token)
model.to(device)
model.eval()
...
for chunk_index in range(0, len(all_contexts) // chunk_size ) : 
  ...
  if len(full_df) == 0 :
    ...
    full_df.to_csv(f"./Llama_responses_{'shuffled' if shuffle else 'unshuffled'}.csv", index=False) # à changer selon le modèle
  else :
