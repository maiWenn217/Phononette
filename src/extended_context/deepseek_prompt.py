...
MODEL_NAME = "deepseek-ai/DeepSeek-R1-Distill-Llama-8B" # à changer selon le modèle
...
for chunk_index in range(0, len(all_contexts) // chunk_size ) : 
  ...
  if len(full_df) == 0 :
    ...
    full_df.to_csv(f"./Deepseek_responses_{'shuffled' if shuffle else 'unshuffled'}.csv", index=False) # à changer selon le modèle
  else :
