...
df = pd.read_json("./fr-extract.jsonl.gz", lines=True, chunksize=20) # le path du fichier Kaikki
...
for chunk in df :
  ...
  for json_entry in json_entries : 
    ...
    response = ollama.chat(
            model="llama3.2", # le nom du modèle
            messages=[
                {
                    "role": "system",
  ...
  if len(full_df) == 0 :
    ...
    full_df.to_csv("./test_llama.csv", index=False) # le path du dossier où le csv doit être sauvegardé
  else :
